...
//changes that should happen are switch to API v2 for all functions such as post tweet, direct messages, etc. Also change all requests to needle
...
TwitterCredentialsNode.prototype.get = function (url, opts) {
  ...
  ////////  
    ...
    // var header = {
    //     headers: {
    //         'Authorization': 'OAuth' + {consumer_key: this.credentials.consumer_key,
    //         consumer_secret: this.credentials.consumer_secret,
    //         token: this.credentials.access_token,
    //         token_secret: this.credentials.access_token_secret }}
// }
...
// this part is related to direct messages
TwitterCredentialsNode.prototype.getUsers = function (users, getBy) {
    ...
    // return this.get("https://api.twitter.com/2/users/lookup.json", params).then(function (result) {
                ...
                var setupStream = function () {
                    if (node.restart) {
                        (async () => {
                            ...
                            console.log("Twitter API is streaming public tweets with search term " + node.tags || " ");
